def main():
	...
	peopleTime = "1200"
	peopleTimeList = []
	sequence_count = (int(data[0]) // 1440) * 67
	iteration_number = int(data[0]) % 1440
	for i in range(int(iteration_number) + 1):
		# print(time[:-2])
		# print(time[-2:])
		...
		print(peopleTime[:-2] + ":" + peopleTime[-2:] + " (" + currentTime[:-2] + ":" + currentTime[-2:] + ")", end="")
		if is_it_arithmetric_sequence(peopleTime):
			sequence_count += 1
			print(" *", end="")
			peopleTimeList.append(peopleTime[:-2] + ":" + peopleTime[-2:] + " *")
		try:
			print(" (" + str(i) + "/" + str(int(iteration_number)) + ") " + str(i / int(iteration_number) * 100) + "%")
		except:
			print()
	print("\n" + str(sequence_count))
